...
for i = 3 : 6
    path = sprintf("Z:\\PAL_XFEL_2509\\ssd2\\ue_250907_FXL\\scratch\\resultsTRXL\\Mn_MeOH\\run00%d\\DiffAve\\",i);
    DIR = dir(fullfile(path,"*.txt"));
    pattern = '[+-]?\d+\.?\d*[eE][+-]?\d+';
    ...
    for j = 1 : length(names)
        subpath = path+sortedFileNames(j);
...
colNames = "T_" + string(sortedTimes); 
...
%%
  U = zeros(length(T_q_cut.q), length(sortedTimes), num_heat_basis);
    S = zeros(length(sortedTimes), length(sortedTimes), num_heat_basis);
    V = zeros(length(sortedTimes), length(sortedTimes), num_heat_basis);
    ...
    surf(q,t, Table','EdgeColor', 'None', 'facecolor', 'interp');
    ...
    xlabel('q')
    ylabel('t')
    title('Original \DeltaS(q, t)')


    figure('Color','white','pos',[10 10 800 800]);
    surf(q,t, heating(:, :, num_heat_sel)','EdgeColor', 'None', 'facecolor', 'interp');
    set(gca,'Ydir','reverse','TickDir','out')
    colormap jet
    zlim(scale)
    clim(scale)
    ...
    xlabel('q')
    ylabel('t')
    title('Heating \DeltaS(q, t)')


    figure('Color','white','pos',[10 10 800 800]);
    surf(q,t, A_correct(:, :, num_heat_sel)','EdgeColor', 'None', 'facecolor', 'interp');
    set(gca,'Ydir','reverse','TickDir','out')
    colormap jet
    zlim(scale)
    clim(scale)

...
parentFolderH5="C:\Users\user\Desktop\PAL_XFEL\rawdata";
parentFolderResults = "C:\Users\user\Desktop\PAL_XFEL\scratch\resultsTRXL";
...
autoAUCValue=0.005; % Manual AUC setting;

%% Options
% % XFEL options
% XrayCenter=12.7018; % KeV % 230107 231218
% XrayCenter=15.0002; % KeV % 230916
% XrayCenter=15.0003; % KeV % 231023
% XrayCenter=14.999654; % KeV % 231024
%XrayCenter=12.2984; % KeV % 240323
XrayCenter=15.0195; % KeV % 240323
...
if isempty(runNumber)
	    pauseTime = 10; % Wating time for new analysis
            ...
            % config.channelROI{5} = {'eh1pdcom_channel0p',1,[]}; % name | dataType : (1 = scalar) | memory
            ...
            if outputWriteOn
                ...
                if config.isAutoRun && ~config.weAreEndGameNow
                    ...
                    if DiffavSave
                        writeOutputDiffav(diffStruct,config);
                    end
            ...
            pathh5=sprintf('%s/pulseInfo',folderh5); % pulseinfo savingissue
            % pathh5=sprintf('%s/eh1rayMXAI_tth',folderh5); 
            % pathh5=sprintf('%s/',folderh5); 
            namePattern=sprintf('%s/*.h5',pathh5);
            h5Struct=dir(namePattern);
            ...
            function [cutoffFig,numGood,R2] = plotCutoff(h5Struct,config)
            %% Section 0 | Decapsulation
            autoAUCValue=config.autoAUCValue;
            numH5=config.numH5;
            I0cutoff = config.I0cutoff;
            cutoffFig=figureWithoutStealing(22222,'cutoff');
            tiledlayout_space_Jkim(4,2);
            ...
            [gradi,y0,R2,~] = plotI0I(I0,I,xlableText,ylableText,'');
            hold on
            ...
            for intIndex=1:numInt+1
                ...
                if intIndex==1
                    ...
                    maxValue=max(abs(SV(intIndex-1,:)));
            ...
            function [figureObj,R2] = plotI0IAll(figureNumber,h5Struct,config)
            ...
            [gradi,y0,R2,R2_adj] = plotI0I(I0,I,xlableText,ylableText,titleText);

            fprintf('  R2    | %.4f, R2(adj.)| %.4f\n',R2,R2_adj);
            fprintf('  gradi | %.4g, offset  | %.4g\n',gradi,y0);
            disp('--------------------------------------------------------------')
            end
            function [gradi,y0,R2,R2_adj] = plotI0I(I0,I,xlabelText,ylabelTxet,titleText)
            ...
            textComment = sprintf('R^2: %.4f | R^2(adj.) %.4f\nGradient: %.4g | Offset: %.4g',R2,R2_adj,p(1),p(2));
            ...
            function writeOutputDiffav(diffStruct,config)
            ...
            for idx=1:numDiff
                ...
                name=sprintf('%s/diff_av_%.3e.txt',folderDiffAve,diffStruct(idx).timedelay);
                fid=fopen(name,'w');
            ...
            fcloseWithComment;
            writeOutputComment(folderDiffAve);
            ...
            function text_Tiled(comment,textSize)
            switch nargin
                case 1
                    textSize = 10;
                case 2
            end
            text(0.5,0.99,comment,'Units','normalized','HorizontalAlignment','center','FontWeight','bold','VerticalAlignment','top','FontSize',textSize);
            end
            ...
            function weAreEndGameNow = DrStrange(numData,numDiff)
            if numData == numDiff
            ...
            fprintf('  Progress status | %.1f%% (%u/%u)\n',numDiff/numData*100,numDiff,numData);
            fprintf('  --------------------------------------------------------------\n');
            end

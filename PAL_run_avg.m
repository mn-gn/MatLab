...
%%
 T = mean(S,3);
 q_delta_S = T;
writematrix(q_delta_S,'qDS_Mn.dat')
%%
...
q_heat = heat_data(:,1);
heat_basis = q_heat.*heat_data(:,2:end);
%%
...
heat_basis_interp = interp1(q_heat,heat_basis,q,"linear","extrap");
...
heat_basis_interp_smooth = smoothdata(heat_basis_interp,1,"movmean",10);
 ...
 A_correct = zeros(size(T,1),size(T,2),size(heat_basis,2));
 heating = A_correct;

 num_heat_basis = size(heat_basis,2);
 for j = 1 : num_heat_basis
     for i = 1 : length(t)
         x = lsqr(heat_basis_interp(:,1:j),T(:,i));
         arti = heat_basis_interp(:,1:j)*x;
         A_correct(:,i,j) = T(:,i)-arti;
         heating(:,i,j) = arti;
     end
 end
%%
...
% end
%%
  U = zeros(length(q), length(t), num_heat_basis);
    ...
    %%
    close all
    for num_heat_sel = 3 : 3
    
    scale = [-0.0226, 0.0226];
    
    % figure('Color','white','pos',[10 10 800 800]);
    % surf(q,t, T','EdgeColor', 'None', 'facecolor', 'interp');
    % set(gca,'Ydir','reverse','TickDir','out','YScale','log')
    % colormap jet
    % zlim(scale)
    % clim(scale)
    % view(2);
